    ...
    <div class="row">
        <div class="col-lg-3 mb-3">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalkategoris"
                onclick="return tambahKategori('{{ route('kategorisAdd') }}')">
                Tambah
            </button>
        </div>
    </div>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    <form action="{{ route('kategorisDelete', ['id' => $p->id]) }}" method="POST"
                                        onsubmit="return confirm('Apa anda yakin untuk menghapus?')">
                                        <button type="button" class="btn p-0"
                                            onclick="return updateKategori('{{ route('kategorisUpdate', ['id' => $p->id]) }}', '{{ $p->id }}')"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="modalkategorisLabel">Form Kategori</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('kategorisAdd') }}" method="post" onsubmit="return confirm('apa anda yakin?')"
                    id="formKategori">
                    @csrf
